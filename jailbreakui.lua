...
pcall(function()
    ...
    local old = parent:FindFirstChild('JailbreakUI')
    if old then old:Destroy() end
    local blur = Lighting:FindFirstChild('JailbreakUIBlur')
...
local function clearBlur()
    ...
    pcall(function()
        for _, c in ipairs(Lighting:GetChildren()) do
            if c:IsA('BlurEffect') and (c.Name == 'JailbreakUIBlur' or string.find(c.Name, 'Jailbreak', 1, true)) then
                c:Destroy()
            end
        end
    end)
...
function Library:CreateWatermark(opts)
    ...
    local titleChip = chip(opts.Title or opts.Name or 'jailbreak', Theme.accent, true)
    chip('|', Theme.textMute)
    ...
    local wm = {
        Frame = frame,
        Name = opts.Title or opts.Name or 'jailbreak',
...
function Library:EnsureWatermark(properties)
    ...
    return Library:CreateWatermark({ Title = properties.Name or properties.Title or 'Jailbreak' })
end
...
function Library:Window(opts)
    opts = opts or {}
    local title = stripRichText(opts.Name or opts.name or 'Jailbreak')
...
function Library:PlayLoading(opts)
    ...
    tween(logo, { Size = UDim2.new(0, 340, 0, 60), BackgroundTransparency = 0 }, 0.45, Enum.EasingStyle.Back)
    ...
    tween(logo, { BackgroundTransparency = 1, Size = UDim2.new(0, 380, 0, 60) }, 0.3)
...
local runDemo = true
pcall(function()
    if getgenv and getgenv().JailbreakLoadingUI then
        runDemo = false
    end
end)

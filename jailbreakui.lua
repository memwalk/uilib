...
pcall(function()
    ...
    local old = parent:FindFirstChild('BapJailbreakUIv2')
    if old then old:Destroy() end
    local blur = Lighting:FindFirstChild('BapJailbreakUIBlur')
...
local function clearBlur()
    ...
    pcall(function()
        for _, c in ipairs(Lighting:GetChildren()) do
            if c:IsA('BlurEffect') and (c.Name == 'BapJailbreakUIBlur' or string.find(c.Name, 'BapJailbreak', 1, true)) then
                c:Destroy()
            end
        end
    end)
...
function Library:CreateWatermark(opts)
    ...
    local titleChip = chip(opts.Title or opts.Name or 'bap jailbreak', Theme.accent, true)
    chip('|', Theme.textMute)
    ...
    local wm = {
        Frame = frame,
        Name = opts.Title or opts.Name or 'bap jailbreak',
...
function Library:EnsureWatermark(properties)
    ...
    return Library:CreateWatermark({ Title = properties.Name or properties.Title or 'Bap Jailbreak' })
end
...
function Library:CreateTargetHUD(opts)
    ...
    local hud = {
        ...
        Update = function(_, data)
            ...
            nameL.Text = 'user  ' .. tostring(data.Name or '—')
            distL.Text = 'dist  ' .. tostring(data.Distance or '—')
            kdL.Text = tostring(data.KD or 'kd  —')
            local status = tostring(data.Status or '—')
            statusL.Text = 'status  ' .. status
            statusL.TextColor3 = data.StatusColor or (status == 'visible' and Theme.success or Theme.danger)
            playL.Text = 'playtime  ' .. tostring(data.Playtime or '—')
            local reports = tonumber(data.Reports) or 0
            reportsL.Text = 'reports  ' .. tostring(reports)
            reportsL.TextColor3 = reports > 0 and Theme.danger or Theme.textDim
            toolL.Text = 'tool  ' .. tostring(data.Tool or '—')
            local visor = tostring(data.Visor or '—')
            visorL.Text = 'visor  ' .. visor
            visorL.TextColor3 = visor == 'on' and Theme.success or Theme.textDim
            local hp = tonumber(data.Health) or 0
            local maxHp = math.max(tonumber(data.MaxHealth) or 1, 1)
...
function Library:Window(opts)
    opts = opts or {}
    local title = stripRichText(opts.Name or opts.name or 'Bap Jailbreak')
    local subtitle = opts.Subtitle or 'jailbreak'
...
function Library:PlayLoading(opts)
    ...
    status.Position = UDim2.new(0.5, 0, 0.42, 28)
    status.Size = UDim2.new(0, 280, 0, 16)

    local barBg = Instance.new('Frame')
    barBg.AnchorPoint = Vector2.new(0.5, 0)
    barBg.Position = UDim2.new(0.5, 0, 0.42, 56)
...
local runDemo = true
pcall(function()
    if getgenv and getgenv().BapJailbreakLoadingUI then
        runDemo = false
    end
end)
...
Library:CreateWatermark({ Title = 'bap jailbreak' })
...
Library:Notify('Bap Jailbreak ready', 2)
end
